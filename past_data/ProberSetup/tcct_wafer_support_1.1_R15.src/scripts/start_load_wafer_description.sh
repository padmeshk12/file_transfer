...
if [ $# -ne 1 ]
then
	echo "$0  *** ERROR! missing Wafer Description File name ***"
	exit 1
fi
...
if [ $SMT_BITS == "64" ]
then
	EXEC=load_wafer_description-64bit
else
	EXEC=load_wafer_description
fi 

echo "Starting load_wafer_description..." 
EXEC_OUTPUT=`/opt/hp93000/soc/system/bin/run -x ${TCCT_WAFER_SUPPORT_HOME}/${EXEC} -- $1`
