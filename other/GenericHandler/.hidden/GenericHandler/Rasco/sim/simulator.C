 ...
 * CREATED  : 28 Aug 2006
 *
 * CONTENTS : Rasco handler simulator
 *
 * AUTHORS  : Xiaofei Han, STS R&D Shanghai, initial revision
 *
 *-----------------------------------------------------------------------------
 *
 * HISTORY  : 28 Aug 2006, Xiaofei Han, Initial Rasco handler driver based on the
 *                                      Multitest handler driver.
 *
 * Instructions:
 *
...
static unsigned char getSrqByte(struct all_t *all)
{
    ...
    phLogSimMessage(all->logId, LOG_VERBOSE, "jump into getsrqbyte");
    ...
    switch ( all->model )
    {
        case SO3000:
                ...
                srqByte= 0xF & (unsigned char)(all->handler->getSitePopulationAsInt());
                    ...
                    // bit 6 Handler is in ready state
            srqByte |= (1<<6) * ( all->deltaStatus->getHandlerIsReady() && !(all->deltaStatus->getStopped()) );
            break;
    }
...
static void service_temp_command_nosend(struct all_t *all, char *m)
{
    phLogSimMessage(all->logId, LOG_VERBOSE, "handling \"%s\"",m);
...
static void service_sites_mapping(struct all_t *all, char *m)
{
    char returnBuffer[MAX_MSG_LENGTH]="";
    strcpy(returnBuffer,"SITE MAPPING: 1 2 3 4;" );
    phLogSimMessage(all->logId, LOG_VERBOSE, "handling \"%s\"", m);

    switch ( all->model ) 
    {
       case SO3000:
         sendMessage(all,returnBuffer);
         break;
    }
}
...
static void handleMessage(struct all_t *all)
{
    ...
    if (phToolsMatch(m, "id\\?"))
	    service_identify(all, m);
    else if (phToolsMatch(m, ":temp:setpoint .*"))
        service_temp_command_nosend(all,m);
    else if (phToolsMatch(m,":temp:setpoint\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, ":temp:soaktime\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "sqb\\?"))
        service_srqbyte(all,m);
    else if (phToolsMatch(m, ":sites:mapping\\?")) 
        service_sites_mapping(all,m);
...
int arguments(int argc, char *argv[], struct all_t *all)
{
	...
	switch (c) 
	{
	  ...
	  case 'm':
	    ...
	    if (strcmp(optarg, "SO3000") == 0) all->model = SO3000;
        else errflg++;
	    break;
      case 's':
	...
	if (errflg) 
	{
	    ...
	    fprintf(stderr, "                         SO3000, default: SO3000\n");
...
//    allg.populate_reprobe_pending_sites=true;
    ...
    allg.deltaStatus->setStopped(false);
    phPbGuiChangeValue(&allg, allg.guiId, "t_stop", "false");
