  ...
  *responseString = response;
  ...
  while(!done)
  {
    ...
    case PHCOM_IFC_GPIB:
      ...
      break;
    case PHCOM_IFC_LAN:
      comRetVal = phComLanSend(f->comId, command, strlen(command),timeout);
      break;
    case PHCOM_IFC_RS232:
      comRetVal = phComRs232Send(f->comId, command, strlen(command),timeout);
      break;
    default:
      phLogFrameMessage(f->logId, PHLOG_TYPE_WARNING,
                        "Unknown connection type.");
      comRetVal = PHCOM_ERR_UNKNOWN;
      break;
    }
  ...
  *responseString = response;
  ...
  while(!done)
  {
    ...
    case PHCOM_IFC_GPIB:
      ...
      break;
    case PHCOM_IFC_LAN:
      comRetVal = phComLanReceive(f->comId, &tmpString, &responseLen, timeout);
      break;
    case PHCOM_IFC_RS232:
      comRetVal = phComRs232Receive(f->comId, &tmpString, &responseLen, timeout);
      break;
    default:
      phLogFrameMessage(f->logId, PHLOG_TYPE_WARNING,
                        "Unknown connection type.");
...
)
{
  ...
  *responseString = response;
  ...
  //firstly check the connection type
  if(f->connectionType != PHCOM_IFC_GPIB)
  {
    phLogFrameMessage(f->logId, PHLOG_TYPE_ERROR,
                      "SRQ query is only available for GPIB connection.");
  ...
  else
  {
      ...
      phLogFrameMessage(f->logId, PHLOG_TYPE_MESSAGE_4,
              "phFrameGetSrqStatusByte invoked by ph_read_status, timeout value for the invocation is: %d", totalTime);
  ...
  while(!done)
  {
    ...
    case PHCOM_ERR_OK:
      retVal = PHTCOM_CI_CALL_PASS;
      phLogFrameMessage(f->logId, PHLOG_TYPE_MESSAGE_4,
                        "Got expected SRQ 0x%02x (%d)", (int)event.d.srq_byte);
  ...
  *responseString = response;
  ...
  while(!done)
  {
    ...
    case PHCOM_IFC_GPIB:
      ...
      break;
    case PHCOM_IFC_LAN:
      comRetVal = phComLanSend(f->comId, (char*)(commandStringWithoutTimeout.c_str()), commandStringWithoutTimeout.length(),timeout);
      break;
    case PHCOM_IFC_RS232:
      comRetVal = phComRs232Send(f->comId, (char*)(commandStringWithoutTimeout.c_str()), commandStringWithoutTimeout.length(),timeout);
      break;
    default:
      phLogFrameMessage(f->logId, PHLOG_TYPE_WARNING,
                        "Unknown connection type.");
