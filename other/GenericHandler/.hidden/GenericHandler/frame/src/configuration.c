    ...
    phConfId_t tempConfId)
{
    ...
    if (goOn)
    {
        ...
        if ((f->funcAvail & PHFUNC_AV_RECONFIGURE))
            if (!doReconfigure(f))
                return 0;
    }
...
)
{
    ...
    char editFileName[32] = "/tmp/tcifileXXXXXX";
	...
	{
	    /* provide editor */
/* Begin of Huatek Modification, Donnie Tu, 04/16/2001 */
/* Issue Number: 201 */
...
)
{
    ...
    if(NULL != strstr(inputString, "handler_site_ids")) //we received site number change request
    {
        ...
        {
            strncpy(defaultSiteMask, "handler_site_mask: [", MAX_SITE_INFO_LENGTH-1);
            strncpy(defaultSiteMapping, "smartest_site_to_handler_site_map: [", MAX_SITE_INFO_LENGTH-1);
            int iLengthMask = 0, iLengthMapping = 0;
            ...
            //final construction
            snprintf(defaultSiteSetting, MAX_SITE_SETTING_STRING, "%s\n%s\n%s", inputString, defaultSiteMask, defaultSiteMapping);

            phLogFrameMessage(f->logId, LOG_NORMAL, "Number of sites will be extend from %d to %d, please also change the site mask, and site mapping for adaption, otherwise, all of these parameters will be fill out with default value:\n%s", currentSiteNumbers, newSiteNumbers, defaultSiteSetting);
