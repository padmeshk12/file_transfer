 ...
 * CREATED  : 14 Sep 2000
 *
 * CONTENTS : Multitest handler simulator
 *
 ...
 *              Modified function service_bin() to fix segmentation fault issues.
 *            24 May 2006, Xiaofei Han, 
 *              1) added function service_sitesquery() to process the SITES query
 *              2) added new model MT9918 and MT9928
 *
 * Instructions:
 *
...
enum model_t { MT9308, MT9320, MT9510, MT9918, MT9928 };
...
unsigned long Handler::getSitePopulationAsLong()
{
    ...
    phLogSimMessage(logId, LOG_DEBUG, "%s", message);
    phLogSimMessage(logId, LOG_VERBOSE, "Handler::getSitePopulationAsLong() returned value is %08lx",sitePop);
    return sitePop;
}
...
static void service_identify(struct all_t *all, char *m)
{
    ...
    {
        case MT9510:
            sprintf(message, "%s Handler 42 Ver %sc Logic", 
                             all->modelname, 
                             all->softwareVersion);
            break;
        case MT9320:
...
static void service_which(struct all_t *all, char *m)
{
    ...
    switch ( all->model )
    {
        ...
        case MT9928:
            ...
            break;
        case MT9320:
            sprintf(message, "%s:%sc 12.0", all->handlerNumber, all->softwareVersion);
            break;
    }
    sendMessage(all, message );
    return;
...
static void service_fullsitesquery(struct all_t *all, char *m)
{
    char messageBuffer[64];
    unsigned long devicesReady = 0;
    phLogSimMessage(all->logId, LOG_VERBOSE, "handling \"%s\"", m);
    devicesReady = all->handler->getSitePopulationAsLong();
    sprintf(messageBuffer, "Fullsites %08lx", devicesReady);
    replyToQuery(all, messageBuffer);
    return;
}

static void service_barcode(struct all_t *all, char *m)
{
    char messageBuffer[64];
    int siteNumber = 0;

    phLogSimMessage(all->logId, LOG_VERBOSE, "handling \"%s\"",m);
    if(sscanf(m,"hvc%d",&siteNumber)!=1){
      phLogSimMessage(all->logId, PHLOG_TYPE_ERROR, "Incorrect site number \"%d\" \n",siteNumber);
      return;
    }

    switch(siteNumber){
    case 1:
      strcpy(messageBuffer, "NNN9868Y0675240712300101");
      break;
    case 2:
      strcpy(messageBuffer, "NNN9868Y0675240712300102");
      break;
    case 3:
      strcpy(messageBuffer, "NNN9868Y0675240712300103");
      break;
    case 4:
      strcpy(messageBuffer, "NNN9868Y0675240712300104");
      break;
    case 5:
      strcpy(messageBuffer, "NNN9868Y0675240712300105");
      break;
    case 6:
      strcpy(messageBuffer, "NNN9868Y0675240712300106");
      break;
    case 7:
      strcpy(messageBuffer, "NNN9868Y0675240712300107");
      break;
    case 8:
      strcpy(messageBuffer, "NNN9868Y0675240712300108");
      break;
    case 9:
      strcpy(messageBuffer, "NNN9868Y0675240712300109");
      break;
    case 10:
      strcpy(messageBuffer, "NNN9868Y0675240712300110");
      break;
    case 11:
      strcpy(messageBuffer, "NNN9868Y0675240712300111");
      break;
    case 12:
      strcpy(messageBuffer, "NNN9868Y0675240712300112");
      break;
    case 13:
      strcpy(messageBuffer, "NNN9868Y0675240712300113");
      break;
    case 14:
      strcpy(messageBuffer, "NNN9868Y0675240712300114");
      break;
    case 15:
      strcpy(messageBuffer, "NNN9868Y0675240712300115");
      break;
    case 16:
      strcpy(messageBuffer, "NNN9868Y0675240712300116");
      break;
    default:
      strcpy(messageBuffer, "InvalidSiteNumber");
      break;
          }
...
static void service_querybins(struct all_t *all, char *m)
{
    ...
    for ( s=0 ; s < all->handler->getNumOfSites() ; ++s)
    {
        ...
        //
        // Offsite bug fixes reprobe is -3 for MT9320
        // if ( all->model==MT9320 && bin==all->handler->getReprobeCategory() )
        // {
        //     bin=-1;
        // }
        //
...
static void handleMessage(struct all_t *all)
{
    ...
    if (phToolsMatch(m, "testpartsready\\?"))
	service_testpartsready(all, m);
    else if (phToolsMatch(m, "tpr\\?"))
	service_testpartsready(all, m);
    else if (phToolsMatch(m, "testparts\\?"))
	service_testpartsready(all, m);
    else if (phToolsMatch(m, "testresults .*"))
	service_testresults(all, m);
    else if (phToolsMatch(m, "trs .*"))
	service_testresults(all, m);
    else if (phToolsMatch(m, "sta!?"))
	service_start(all, m);
    else if (phToolsMatch(m, "start!?"))
	service_start(all, m);
    else if (phToolsMatch(m, "identify\\?"))
	service_identify(all, m);
    else if (phToolsMatch(m, "id\\?"))
	service_identify(all, m);
    else if (phToolsMatch(m, "which\\?"))
	service_which(all, m);
    else if (phToolsMatch(m, "whi\\?"))
	service_which(all, m);
    else if (phToolsMatch(m, "numtestsites\\?"))
        service_numtestsites(all,m);
    else if (phToolsMatch(m, "nts\\?"))
        service_numtestsites(all,m);
    else if (phToolsMatch(m, "sitespacing\\?"))
        service_sitespacing(all,m);
    else if (phToolsMatch(m, "set sitedisable\\?"))
        service_sites_disabled(all, m);
    else if (phToolsMatch(m, "sd\\?"))
        service_sites_disabled(all, m);
    else if (phToolsMatch(m, ".* killchuck\\?"))
        service_killchuck(all,m);
    else if (phToolsMatch(m, "verifytest\\?"))
        service_verifytestQuery(all,m);
    else if (phToolsMatch(m, "verf\\?"))
        service_verifytestQuery(all,m);
    else if (phToolsMatch(m, "verifytest .*"))
        service_verifytestCommand(all,m);
    else if (phToolsMatch(m, "verf .*"))
        service_verifytestCommand(all,m);
    else if (phToolsMatch(m, "cmdreply\\?"))
        service_cmdReplyQuery(all,m);
    else if (phToolsMatch(m, "cr\\?"))
        service_cmdReplyQuery(all,m);
    else if (phToolsMatch(m, "cmdreply .*"))
        service_cmdReplyCommand(all,m);
    else if (phToolsMatch(m, "cr .*"))
        service_cmdReplyCommand(all,m);
    else if (phToolsMatch(m, "systemmode\\?"))
        service_systemModeQuery(all,m);
    else if (phToolsMatch(m, "smd\\?"))
        service_systemModeQuery(all,m);
    else if (phToolsMatch(m, "systemmode .*"))
        service_systemModeCommand(all,m);
    else if (phToolsMatch(m, "smd .*"))
        service_systemModeCommand(all,m);
    else if (phToolsMatch(m, "emulationmode\\?"))
        service_emulationModeQuery(all,m);
    else if (phToolsMatch(m, "emm\\?"))
        service_emulationModeQuery(all,m);
    else if (phToolsMatch(m, "emulationmode .*"))
        service_emulationModeCommand(all,m);
    else if (phToolsMatch(m, "emm .*"))
        service_emulationModeCommand(all,m);
    else if (phToolsMatch(m, "errorclear.*"))
        service_errorclear(all,m);
    else if (phToolsMatch(m, "ecl.*"))
        service_errorclear(all,m);
    else if (phToolsMatch(m, "testermode .*"))
        service_general(all,m);
    else if (phToolsMatch(m, "tmd .*"))
        service_general(all,m);
    else if (phToolsMatch(m, "setpoint .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* setpoint .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "setpoint\\?"))  /* CR33707 */
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* sp .*"))
        service_general(all,m);
    else if (phToolsMatch(m, "soaktime .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "soaktime\\?"))  /* CR33707 */
        service_temp_command(all,m);
    else if (phToolsMatch(m, "skt .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "desoaktime .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "desoaktime\\?"))  /* CR33707 */
        service_temp_command(all,m);
    else if (phToolsMatch(m, "dskt .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "tempcontrol .*"))
        service_general(all,m);
    else if (phToolsMatch(m, "tempcontrol\\?"))  /* CR33707 */
        service_temp_command(all,m);
    else if (phToolsMatch(m, "testtemp .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "testtemp\\?"))  /* CR33707 */
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* cool .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "cool\\?"))  /* CR33707 */
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* heat .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* lowerguardband .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "lowguard .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* lowguard .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "highguard .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* highguard .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, ".* upperguardband .*"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "categorysource .*"))
        service_temp_command(all,m);
    // mt99x8 temperature related CR42643
    else if (phToolsMatch(m, "tmp\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "settmp\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "sns( |\\?)[0-9]+\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "tmpch\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "tmpcs\\?"))
        service_temp_command(all,m);
    else if (phToolsMatch(m, "tmp( |!)([0-9]+,[+-]*[0-9]+|off)$"))
        service_temp_command_no_reply(all,m);
    else if (phToolsMatch(m, "st( |!)[0-9]+$"))
        service_temp_command_no_reply(all,m);
    else if (phToolsMatch(m, "atgb( |!)[0-9]+,[0-5],[0-5]$"))
        service_temp_command_no_reply(all,m);
    // mt99x8 temperature related end
    else if (phToolsMatch(m, "status\\?"))
        service_status(all,m);
    else if (phToolsMatch(m, "srqbyte\\?"))
        service_srqbyte(all,m);
    else if (phToolsMatch(m, "sqb\\?"))
        service_srqbyte(all,m);
    else if (phToolsMatch(m, "sites\\?")) 
        service_sitesquery(all,m);
    else if (phToolsMatch(m, "fullsites\\?"))
      service_fullsitesquery(all,m);
    else if (phToolsMatch(m, "hvc([0-9]+)"))
            service_barcode(all,m);
    else if (phToolsMatch(m, "querybins\\?"))
        service_querybins(all,m);
    else if (phToolsMatch(m, "releaseparts!?"))
        service_releaseparts(all,m);
    else if (phToolsMatch(m, "stop!?"))
        service_stop(all,m);
    else if (phToolsMatch(m, "sto!?"))
        service_stop(all,m);
    else if (phToolsMatch(m, "srqmask!? .*"))
        service_srqmask(all,m);
    else if (phToolsMatch(m, "sqm!? .*"))
        service_srqmask(all,m);
    else if (phToolsMatch(m, "srqmask\\?"))
        service_srqmaskQuery(all,m);
    else if (phToolsMatch(m, "srq\\?"))
        service_srqmaskQuery(all,m);
...
/* FLEX COMMANDS */
    ...
    else if (phToolsMatch(m, "retract .*"))
        service_retract(all,m);
    else if (phToolsMatch(m, "contact .*"))
        service_contact(all,m);

/* SUMMIT COMMANDS */
    else if (phToolsMatch(m, "reprobe!? .*"))
        service_reprobe_XX(all,m);
    else if (phToolsMatch(m, "recipestart .*"))
        service_recipestart(all,m);
    else if (phToolsMatch(m, "recipeend .*"))
        service_recipeend(all,m);
    else if (phToolsMatch(m, "recipe activategrp .*"))
        service_general(all,m);

/* RFS COMMANDS */

    else if (phToolsMatch(m, "conehdnum\\??"))
        service_numtestsites(all,m);
    else if (phToolsMatch(m, "chn\\??"))
        service_numtestsites(all,m);
    else if (phToolsMatch(m, ".* chuckdisable\\?"))
        service_site_disabled(all,m);
    else if (phToolsMatch(m, ".* cds\\?"))
        service_site_disabled(all,m);
...
int arguments(int argc, char *argv[], struct all_t *all)
{
	...
	switch (c) 
	{
	  ...
	  case 'm':
	    ...
	    if (strcmp(optarg, "MT9510") == 0) all->model = MT9510;
	    else if (strcmp(optarg, "MT9320") == 0) all->model = MT9320;
	    else if (strcmp(optarg, "MT9308") == 0) all->model = MT9308;
        else if (strcmp(optarg, "MT9918") == 0) all->model = MT9918;
        else if (strcmp(optarg, "MT9928") == 0) all->model = MT9928;
	    else errflg++;
	    break;
          case 's':
...
//
	...
	if (errflg) 
	{
	    ...
	    fprintf(stderr, "                         MT9510, MT9320, MT9308, MT9918, MT9928, default: MT9510\n");
