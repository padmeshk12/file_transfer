...
GpibCommunication::commError HandlerController::tryReceiveMsg(int to) 
{
    ...
    phLogSimMessage(logId, LOG_VERBOSE, "Entering tryReceiveMsg(%d)\n", to);
    ...
    phLogSimMessage(logId, LOG_VERBOSE, "  tryReceiveMsg() after receive, commErr = %d\n", commErr);
...
void HandlerController::sendSRQ(unsigned char x)
{
    ...
    } while(( commErr != GpibCommunication::comm_ok ) && ( ++retries < 3 ));
    
    if ( commErr != GpibCommunication::comm_ok )
    {
        pViewController->setGpibStatus(ViewController::timedOut);
        phLogSimMessage(logId, LOG_DEBUG, "sendSRQ gave up after %d retries", retries);
    }
    else {
        phLogSimMessage(logId, LOG_DEBUG, "sendSRQ sent 0x%X", x);
    }
}
...
void HandlerController::sendMessageEoc(const char* msg, const char* eoc)
{
    ...
    {
        pViewController->setGpibStatus(ViewController::timedOut);
        phLogSimMessage(logId, LOG_DEBUG, "sendMessageEoc gave up after %d retries", retries);
    }
    else 
    {
        phLogSimMessage(logId, LOG_DEBUG, "sendMessageEoc sent OK");
    }
}
...
GpibCommunication::commError HandlerController::trySendMsg(const char* msg, const char* eoc)
{
    ...
    {
      case GpibCommunication::comm_ok:
        phLogSimMessage(logId, LOG_DEBUG, "message send comm_ok, sent: '%s'", msg);
        pViewController->setSendMsgField("");
        break;
      case GpibCommunication::comm_timeout:
        phLogSimMessage(logId, LOG_DEBUG, "message send comm_timeout");
        pViewController->setGpibStatus(ViewController::timedOut);
        break;
      case GpibCommunication::comm_error:
        phLogSimMessage(logId, LOG_DEBUG, "message send comm_error");
        pViewController->setGpibStatus(ViewController::error);
        break;
    }
    return commErr;
