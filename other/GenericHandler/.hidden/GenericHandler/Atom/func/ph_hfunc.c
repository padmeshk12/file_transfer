 ...
 * Authors: Zuria Zhu
 *
 * History: 01 Jul 2023, Zuria Zhu, generic template built
 *
...
static int reconfigureGlobal(struct phFuncStruct *myself)
{
	...
	else
	{
	    ...
	    {
                confError = phConfConfString(myself->myConf, PHKEY_BI_HBIDS, 1, &i, &(myself->binIds[i]));
	    }
            if (confError != PHCONF_ERR_OK) {
              phLogFuncMessage(myself->myLogger, PHLOG_TYPE_ERROR,
                  "binId list failed to get!");
            }
	}
    }
