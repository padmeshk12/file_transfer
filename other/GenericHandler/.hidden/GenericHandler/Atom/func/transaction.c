...
)
{
    ...
    char command[PHESTATE_MAX_SITES * 16];
	...
	"trying to send message to handler: \"%s\", timeout = %ld", command, handlerID->heartbeatTimeout);
    ...
    else if(handlerID->interfaceType == PHFUNC_IF_LAN || handlerID->interfaceType == PHFUNC_IF_LAN_SERVER)
    {
      ...
      comError = phComLanSend(handlerID->myCom, command, strlen(command), handlerID->heartbeatTimeout);
    }
                             ...
                             )
{
    ...
    {
        phLogFuncMessage(handlerID->myLogger, LOG_VERBOSE,
                         "NOT trying to receive message from handler, already received");
        return PHFUNC_ERR_OK;
    }
...
//                    retVal = PHFUNC_ERR_ANSWER;
            ...
            if(strcmp(format,"%s")==0)
            {
                ...
                /* This is a workaround solution to fix unexpeted answer which got from handler CR-156503*/
                /* sometimes, there is \r\n at the begnning of answer, it should be trim*/ 
                if(strncmp(handlerID->cmodelName, "TFS", strlen("TFS")) == 0)
                {
                  char *answer_local = 0;
                  if (answer != NULL) {
                    answer_local = (char*) answer;
                  }
                  if(answer_local!=NULL){

                    if(strlen(answer_local)>0 && (answer_local[0]=='\r' || answer_local[0]=='\n'))
                    {
                      answer_local[0] = ' ';
                    }
                    if(strlen(answer_local)>1 && (answer_local[1]=='\r' || answer_local[1]=='\n'))
                    {
                      answer_local[1] = ' ';
                    }
                    strcpy(p,answer_local);
                  }
                }
                else
                {
