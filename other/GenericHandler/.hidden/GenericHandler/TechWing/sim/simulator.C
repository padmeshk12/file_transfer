 ...
 * CREATED  : 15 Feb 2000
 *
 * CONTENTS : TechWing GPIB handler simulator
 *
...
void initializeDefaults(TechWingHandlerControllerSetup   &simSetup,
                        ViewControllerSetup        &vcSetup)
{
    simSetup.model                            =   TechWingHandlerController::TW2XX;
...
int arguments(int argc, char *argv[], 
              TechWingHandlerControllerSetup   &simSetup,
              ViewControllerSetup      &vcSetup)
{
    int c;
    int errflg = 0;
	...
	switch (c) 
	{
	  ...
	  case 'm':
	    if (strcmp(optarg, "TW2XX") == 0) 
            {
                simSetup.model = TechWingHandlerController::TW2XX;
            }
	    else if (strcmp(optarg, "TW3XX") == 0)
            {
                simSetup.model = TechWingHandlerController::TW3XX;
            }
            else
                errflg++;
	    break;
...
//
	...
	if (errflg) 
	{
	    ...
	    fprintf(stderr, " -m <prober model>       prober model, one of TW2XX, TW3XX\n");
	    fprintf(stderr, "                         default: TW2XX\n");
...
int main(int argc, char *argv[])
{
    ...
    TechWingHandlerControllerSetup   TechWingSimSetup;
    ViewControllerSetup        vcSetup;
    int i;
    ...
    /* init defaults */
    initializeDefaults(TechWingSimSetup, vcSetup);

    /* get real operational parameters */
    if (!arguments(argc, argv, TechWingSimSetup,vcSetup))
	exit(1);

    sleep(1);

    try {
        TechWingHandlerController TechWinghandler(&TechWingSimSetup,&vcSetup);

        TechWinghandler.switchOn();
    }
    catch ( Exception &e )
    {
