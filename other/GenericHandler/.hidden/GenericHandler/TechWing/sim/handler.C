...
void Handler::start()
{
    ...
    if ( status==stopped )
    {
        /* tell simulator to start sending device start SRQ */
...
bool Handler::sendLotStartSignal()
{
    bool sendLotStart = false;
    
    phLogSimMessage(logId, LOG_DEBUG, 
                    "Entering Handler::sendLotStartSignal, \n  autoSetupInit = %d, autoSetupDelayMsec = %g, status = %x, running = %x\n",
                    autoSetupInit, autoSetupDelayMsec, status, running);

    if ( !autoSetupInit && autoSetupDelayMsec > 0 )
    {
        phLogSimMessage(logId, LOG_VERBOSE,
                        "Handler::sendLotStartSignal automatic start time so far %lf wait %lf ",
...
bool Handler::sendTestStartSignal()
{
    ...
    phLogSimMessage(logId, LOG_VERBOSE, 
                    "Entering Handler::sendTestStartSignal, \n  autoSetupInit = %d, autoSetupDelayMsec = %g, status = %x, running = %x\n",
                    autoSetupInit, autoSetupDelayMsec, status, running);
    ...
    switch ( status )
    {
        ...
        case handlingDevices:
            if ( (binningTimer->getElapsedMsecTime() >= handlingDelayMsec) && (running) )
            {
                ...
                {
                    phLogSimMessage(logId, LOG_DEBUG, 
                                    "Handler::sendTestStartSignal, populateSites() returned TRUE");
                    sendTestStart=true;
                    status=waiting;
                }
                else
    ...
    phLogSimMessage(logId, LOG_VERBOSE, 
                    "Handler::sendTestStartSignal is %s ",
...
void Handler::setSitesEnabled(long long mask)
{
    siteEnabledMask = mask;
}


long long Handler::getSitesEnabled()
{
    return siteEnabledMask;
}

long long Handler::getSitesDisabled()
{
...
long long Handler::getNumberOfSitesAsLong()
{
    long long numOfSitesAsLong=0;

    for ( int i=0 ; i< numOfSites ; ++i )
    {
        numOfSitesAsLong+= 1LL << i;
    }
