...
/*****************************************************************************
 *
 * Find external bin number for 32 bin
 *
 * Authors: Adam Huang
 *
 * Description: 
 * Find first external character(0-9, a-w) in string or return null 
 *
 *****************************************************************************/
const char *findExternalBinCode(const char *s)
{
    while (*s && !phToolsMatch(s, "[0-9|a-w].*"))
    {
        ++s;
    }
...
/*****************************************************************************
 *
 * Convert external character into int
 *
 * Authors: Adam Huang
 *
 * Description: 
 * converts external character into an integer
 *
 *****************************************************************************/
int convertExternalBinCodeToInt(const char *s, int binningFormat)
{
    int i_ret = 0;
    if (binningFormat == 32)
    {
    ...
    else
    {
        ...
        i_ret = strtol(decStr, (char **)NULL, 10);
    }
