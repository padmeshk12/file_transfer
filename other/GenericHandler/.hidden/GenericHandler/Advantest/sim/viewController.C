...
{
    static int ictrlrandomflag = 1;
    static int ictrlgpio = -1;
    char message[MAX_MSG_LENGTH];
    ...
    switch (guiEvent.type)
    {
      ...
      case PH_GUI_EXITBUTTON_EVENT:
        ...
        else if(strcmp(guiEvent.name, "p_thermal_alarm") == 0)
        {
            unsigned char x=72;
            pHandlerController->trySendSRQ(x);
#if 0
            if(-1 == ictrlgpio)
            {
                //first push button trigger thermal alarm
                unsigned char x=96;
                pHandlerController->trySendSRQ(x);
            }
            if(true == this->gpioTest)
            {
                INST id;
                ionerror(I_ERROR_EXIT);
                id = iopen("ngpio");                // use GPIO
                itimeout(id, 10000);                // set timeout value to 10 sec
                itrigger(id);                       // Pulses the CTL0 control line.
                igpiosetwidth(id,16);               // sets 8 or 16 bit data width

                if (ictrlgpio == -1 || ictrlgpio == 1)
                {
                    igpioctrl(id, I_GPIO_DATA, 0xFFFF);
                    iclose(id);
                    ictrlgpio = 0;
                    phLogSimMessage(logId, LOG_NORMAL, "set the data line");
                }
                else
                {
                    igpioctrl(id, I_GPIO_DATA, 0x0000);
                    iclose(id);
                    ictrlgpio = 1;
                    phLogSimMessage(logId, LOG_NORMAL, "unset the data line");
                }
            }
#endif
        }
        else if(strcmp(guiEvent.name, "p_random") == 0)
        {
            if (ictrlrandomflag == 1)
            {
                iRandomFlag = rand()%10+1;
                ictrlrandomflag = 0;
                phLogSimMessage(logId, LOG_NORMAL, "randomly send enabled");
            }
            else
            {
                iRandomFlag = 0;
                ictrlrandomflag = 1;
                phLogSimMessage(logId, LOG_NORMAL, "randomly send disabled");
            }
        }
        break;
    }
...
int ViewController::getRandomFlag()
{
    return iRandomFlag;
}

void ViewController::setRandomFlag(int iflag)
{
    iRandomFlag = iflag;
}
