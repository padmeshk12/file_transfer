...
GpibCommunication::commError HandlerController::trySendSRQ(unsigned char x) 
{
    ...
    //record time info when thermal alarm occurs
    struct timeval timeStamp;
    if( 96 == x ) {
        gettimeofday(&timeStamp, NULL);
        phLogSimMessage(logId, LOG_DEBUG, "begin SRQ send at %d(us), %d(s)", timeStamp.tv_usec, timeStamp.tv_sec);
    }
    commErr=pGpibComm->sendEvent(&sendEvent, 1000L*timeout);
    gettimeofday(&timeStamp, NULL);
    phLogSimMessage(logId, LOG_DEBUG, "end SRQ send at %d(us), %d(s)", timeStamp.tv_usec, timeStamp.tv_sec);
    ...
    {
      case GpibCommunication::comm_ok:
        phLogSimMessage(logId, LOG_NORMAL, "SRQ send 0x%02x", int(x));
