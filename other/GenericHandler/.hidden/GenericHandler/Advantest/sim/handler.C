 ...
 *-----------------------------------------------------------------------------
 *
 * HISTORY  : 29 Jan 2014, Magco Li , created
 *
 * Instructions:
 *
...
int Handler::getNumOfDevicesToTest()
{
    return numOfDevicesToTest;
}
...
unsigned long Handler::getSitePopulationAsLong()
{
    ...
    phLogSimMessage(logId, LOG_DEBUG, "Handler::getSitePopulationAsLong() returned value is %08x",sitePop);
...
void Handler::binDataVerified()
{
    ...
    {
        phLogSimMessage(logId, LOG_DEBUG, "alarm handler"); 
    }
}
