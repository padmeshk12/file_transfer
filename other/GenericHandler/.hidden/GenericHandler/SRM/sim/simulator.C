 ...
 * CREATED  : 15 Dec 2010
 *
 * CONTENTS : SRM handler simulator
 *
 * AUTHORS  : Xiaofei Han, STS R&D, initial revision
 *
 *-----------------------------------------------------------------------------
 *
 * HISTORY  : 15 Dec 2010, Xiaofei Han, Created
 *
 * Instructions:
 *
...
public:
    ...
    void setStatu(int statu,bool value);
...
public:
    ...
    int sendDeviceToBin(int s, int b);
    ...
    void reset();
    void handleDevices();
    void setStatus(int statu,bool value);
...
int Handler::sendDeviceToBin(int s, int b)
{
...
void Handler::setStatus(int statu,bool value)
{
  Std::vector<Site>::iterator i; 
  for(i=siteList.begin();i!=siteList.end();++i )
  {
        i->setStatu(statu,value);
  }
}
...
int Site::sendDeviceToBin(int b)
{
    if ( status==empty )
    {
        phLogSimMessage(logId, LOG_VERBOSE,
...
void Site::setStatu(int statu,bool value)
{
    if(value)
    { 
      if(statu == empty)
        status = empty;
      else if(statu == waitingBinData)
        status = waitingBinData;  
      else if(statu == waitingRelease)
        status = waitingRelease;
      else
        phLogSimMessage(logId, LOG_DEBUG, "there is no statu:%d in Site",statu); 
    }
    else 
        phLogSimMessage(logId, LOG_DEBUG, "will not set status"); 
}
...
static void service_testresults(struct all_t *all, char *m)
{
    ...
    do {
        ...
        if ( (posMsg != NULL) && (sscanf(posMsg,",%d",&bin) != 1) )
        {
            phLogSimMessage(all->logId, LOG_VERBOSE,
...
{
    phLogSimMessage(all->logId, LOG_DEBUG, "handling \"%s\"",m);
    check_flags(all, F_STOP | F_JAM | F_DOOROPEN );
    ...
    while ( ( posMsg != NULL ) && (*posMsg) && !errflg )
    {
        ...
        /* get category [a-h] from message string */
        if ( sscanf(posMsg,"bsort%c%d",&c,&bin) == 2 )
        {
            site=c-'a';
        }
        else
        {
            ++errflg;
            phLogSimMessage(all->logId, LOG_VERBOSE,
                "Failed to get binning info from \"%s\" with sscanf \n"
                "expected format bSORTxn",
                posMsg);
        }
        ...
        if ( !errflg )
        {
            if ( all->verifyTest )
            {
              phLogSimMessage(all->logId, LOG_DEBUG,"setBinData");
              all->handler->setBinData(site,bin);
            }
            else
            {
              phLogSimMessage(all->logId, LOG_DEBUG,"sendDeviceToBin");
              binResult = all->handler->sendDeviceToBin(site,bin);
              if(binResult != 0)
                 ++errflg;
            }
        }
        else
            phLogSimMessage(all->logId, LOG_DEBUG,"errflg:%d",errflg);
...
/* FLEX COMMANDS */
    ...
    else if (phToolsMatch(m, "bsort[a-z][0-9]"))
        service_bin(all,m);
...
static void doWork(struct all_t *all)
{
	...
	beAlife(all);
        ...
        if ( all->handler->sendTestStartSignal() )
        {
            ...
            all->handler->setStatus(Site::waitingBinData,true);
        }
...
int arguments(int argc, char *argv[], struct all_t *all)
{
	...
	switch (c) 
	{
	  ...
	  case 'm':
	    ...
	    if (strcmp(optarg, "SRM") == 0) all->model = SRM;
	    else errflg++;
	    break;
          case 's':
...
//
	...
	if (errflg) 
	{
	    ...
	    fprintf(stderr, "                         SRM, default: SRM\n");
