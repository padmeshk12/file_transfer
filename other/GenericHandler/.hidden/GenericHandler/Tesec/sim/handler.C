...
{
    numOfSites               =   4;
    siteEnabledMask          =   allSitesEnabled;
    autoSetupDelay           =   0.0;
    handlingDelay            =   1000;
    numOfDevicesToTest       =   1000;
    pattern                  =   Handler::all_sites_working;
    handlerReprobeMode       =   Handler::ignore_reprobe;
    retestCat                =   -1;
    reprobeCat               =   -2;
    maxVerifyCount           =   2;
    corruptBinDataScheme     =   Handler::none;
}
        ...
        mIsStartFor5170(false)
{
    ...
    setRetestCategory(setup.retestCat);
    setReprobeCategory(setup.reprobeCat);
...
void Handler::setSitePopulated(int s,bool p)
{
   Std::vector<Site>::iterator i;
   if(s < 0 || s >= getNumOfSites() )
    {
        phLogSimMessage(logId, PHLOG_TYPE_ERROR,
                        "Handler::setSitePopulated() site number out of range %d (0-%d)",
                        s,(getNumOfSites()-1));

        // Future enhancement would be to throw an exception
    }
    else{
       i=siteList.begin() + s;
       i->setSitePopulated(p);
       phLogSimMessage(logId,LOG_DEBUG,"Site's status %d has been set to \"%s\"" ,
                        s+1,i->getSitePopulated()?"not empty":"empty");   
    }
}


bool Handler::getSitePopulated(int s)
{
    Std::vector<Site>::iterator i;
    bool sitePopulated=false;
    ...
    {
        phLogSimMessage(logId, PHLOG_TYPE_ERROR, 
                        "Handler::getSitePopulated() site number out of range %d (0-%d)",
                        s,(getNumOfSites()-1));
    ...
    {
        i=siteList.begin() + s;
        sitePopulated=i->getSitePopulated();
        phLogSimMessage(logId, LOG_VERBOSE, "Handler::getSitePopulated(%d) is %s ",
        s,
        (sitePopulated) ? "true" : "false");
    }
...
void Handler::getSitePopulationAsLong_Tesec3270(char *sitepop)
{
    ...
    phLogSimMessage(logId, LOG_VERBOSE, "Handler::getSitePopulationAsLong_Tesec3270()");

    strcpy(message,"Site: ");
/****************************************************************/

    char tmpsitePop[4];
    /*  change the 4f4f4f4f to OOOO 
      ...
      for ( i=siteList.begin(), bitPosition=0; i != siteList.end() ; ++i, ++bitPosition )
    {
        ...
        if ( i->getSitePopulated() )
        {
            sitePop |= ( 1 << (bitPosition+(bitPosition/4)*4 ) );
        }
    }
    ...
    */
    ...
    // set high 4-bit to 0100 of every byte
    //  change sitePop to char char string
    int ii=0;
   // int sitenum[4]={0,0,0,0};
    char flag=0x0f;
    char *p;
    p=(char *)&sitePop;
    int highdef=0;
   // char sitepop[16];
   //  The follow is the case that population set the first byte received as the low bit   
    p++;
    p++;
    p++;
    int ch;
    ...
    {
      ch=((*p)&flag);
         phLogSimMessage(logId, LOG_DEBUG, "%d", ch);

      if(ch >0 )
      {
        highdef=1;
    //    sprintf(tmpsitePop,"%c",(char)( 0x40+ch) );
    //    phLogSimMessage(logId, LOG_DEBUG, "%s", tmpsitePop);        
    //    strcat(sitepop,tmpsitePop);
        if(ii!=3)
           p--;
      }
      else
   ...
   /* if we want to population stores the DUT as high site in high bit, low site in low bit, first strcat the first four site information*/
  ...
  case 0:
    for(ii=0;ii<4;ii++){
       sprintf(tmpsitePop,"%c",(char) (0x40+ii+1) );
            strcat(sitepop,tmpsitePop);
    }
  break;
  case 1:
    for(ii=0;ii<4;ii++){
       sprintf(tmpsitePop,"%c",(char) (0x40+ii*2+1) );
            strcat(sitepop,tmpsitePop);
    }
  break;
  }
  
    phLogSimMessage(logId, LOG_VERBOSE, "Handler::getSitePopulationAsLong() returned value is %s",sitepop);
   

        
   //
   // unsigned long _send_command  = 0x40404040;
   // sitePop |= _send_command;
    // end of set
 ...
 phLogSimMessage(logId, LOG_DEBUG, "%s", sitepop);
   ...
   // end of edit send message after handler 3270 receiving Ack message " D "
  // return sitepop;
}
...
void Handler::start()
{
     phLogSimMessage(logId, LOG_DEBUG,"Enter start");
    binningTimer->start();
    if ( status==stopped )
    {
...
void Handler::setStartFlagFor5170(bool flag)
{
    mIsStartFor5170 = flag;
}

bool Handler::isStartFor5170()
{
    return mIsStartFor5170;
}
void Handler::stop()
{
       phLogSimMessage(logId, LOG_DEBUG,"Enter stop");
    running=false;
}
...
bool Handler::sendTestStartSignal()
{
    ...
    if ( firstTimeStart == -1 && sendTestStart )
    {
        firstTimeStart=1;
    }
...
bool Handler::testsNotStarted()
{
    return ( firstTimeStart  == 1 );
}
