 ...
 * CREATED  : 15 Feb 2000
 *
 * CONTENTS : Tesec GPIB handler simulator
 *
...
void initializeDefaults(TesecHandlerControllerSetup   &simSetup,
                        ViewControllerSetup           &vcSetup)
{
    simSetup.model                            =   TesecHandlerController::T9588IH;
...
int arguments(int argc, char *argv[], 
              TesecHandlerControllerSetup   &simSetup,
              ViewControllerSetup           &vcSetup)
{
    int c;
    int errflg = 0;
	...
	switch (c) 
	{
	  ...
	  case 't':
	    ...
	    break;
	  case 'm':
	    if (strcmp(optarg, "9588") == 0) 
            simSetup.model = TesecHandlerController::T9588IH;
        else if(strcmp(optarg, "3270") == 0)
            simSetup.model = TesecHandlerController::T3270IH;
        else if( strcmp(optarg, "5170") == 0)
            simSetup.model = TesecHandlerController::T5170IH;
	    else 
                errflg++;
	    break;
          case 's':
	...
	if (errflg) 
	{
	    ...
	    fprintf(stderr, "                         3270,9588,5170 default: 9588\n");
...
int main(int argc, char *argv[])
{
    ...
    TesecHandlerControllerSetup   TSimSetup;
    ViewControllerSetup           vcSetup;
    int i;
    ...
    try {
        TesecHandlerController TesecHandler(&TSimSetup,&vcSetup);

        TesecHandler.switchOn();
    }
    catch ( Exception &e )
    {
