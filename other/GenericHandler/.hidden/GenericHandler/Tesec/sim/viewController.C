     ...
     alivePulse(true)
{
    phLogSimMessage(logId, LOG_VERBOSE,
                    "ViewController constructor called: with handler %P",
                    pHandCont);
    timeout_getGuiData=1;
}
...
void ViewController::createView()
{
    ...
    if ( !guiId && !noGui )
    {
        ...
        char *descr;
        int debug;
        const char *title="GPIB Handler";
 
        descr = new char[strlen(viewControllerDesc) + 256];
...
void ViewController::handleEvent(const phGuiEvent_t &guiEvent)
{
    ...
    switch (guiEvent.type)
    {
      ...
      case PH_GUI_EXITBUTTON_EVENT:
        ...
        {
            char name[64] = "";
            strcpy(name,pHandlerController->getModelName());
            if(strstr(name,"5170"))
                pHandlerController->pHandler->setStartFlagFor5170(true);
            else
