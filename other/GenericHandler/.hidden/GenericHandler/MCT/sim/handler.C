...
void Handler::getSitePopulationAsLong_MCTFH1200(char *sitepop)
{
    int site = 0;
    char message[MAX_MSG_LENGTH];
    char tmpString[64];
    ...
    phLogSimMessage(logId, LOG_VERBOSE, "Handler::getSitePopulationAsLong_MCTFH1200()");
...
/****************************************************************/
    ...
    for ( i=siteList.begin(), bitPosition=0; i != siteList.end() ; ++i, ++bitPosition )
    {
        ...
        if ( i->getSitePopulated() )
        {
            ...
            sprintf(tmpsitePop,"%c%c%d",(char) (0x40+1),(char)(0x40+2+xPos++), yPos );
...
void Handler::logHandlerStatistics()
{
    ...
    char lineBuffer[MAX_MSG_LENGTH];
    char tmpString[64];
    s=0;
    strcpy(lineBuffer,"         ");
    for ( si=siteStatList.begin() ; si != siteStatList.end() ; ++si )
