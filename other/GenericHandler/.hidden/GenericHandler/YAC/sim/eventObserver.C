    ...
    pHandController(pHC)
{
    ...
    pYACStatus->registerRecorder(this,recObs);
}
...
void EventPlayback::checkEvent(int eventCount)
{
    while ( eventCount >= recordStep && !end )
    {
        ...
        {
            bool eventUsed=pYACStatus->setStatusBit(this, recordStatus, recordValue);
...
void EventPlayback::YACStatusChanged(YACStatus::eYACStatus, bool b)
{
}
...
void EventPlayback::getPlaybackComponent()
{
    ...
    if ( rtn_fscanf == 3 )
    {
        ...
        recordStatus=pYACStatus->getEnumStatus(strRecordName);
