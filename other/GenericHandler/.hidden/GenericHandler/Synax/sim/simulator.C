 ...
 * CREATED  : 15 Feb 2000
 *
 * CONTENTS : Synax-GPIB handler simulator
 *
 ...
 *            11 Mar 2005, Ken Ward, copied from Seiko-Epson version
 *
 * Instructions:
 *
...
void initializeDefaults(SXHandlerControllerSetup   &simSetup,
                        ViewControllerSetup        &vcSetup)
{
  simSetup.model                            =   SXHandlerController::SX1101;
...
int arguments(int argc, char *argv[], 
              SXHandlerControllerSetup   &simSetup,
              ViewControllerSetup      &vcSetup)
{
  int c;
  int errflg = 0;
  ...
  while((c = getopt(argc, argv, ":d:i:t:m:s:M:p:V:D:A:N:ISo:P:T:S:QZn")) != -1) {
    switch(c) {
      ...
      case 'm':
        if(strcmp(optarg, "SX1101") == 0) {
          simSetup.model = SXHandlerController::SX1101;
        } else if(strcmp(optarg, "SX1701") == 0) {
          simSetup.model = SXHandlerController::SX1701;
        } else if(strcmp(optarg, "SX2400") == 0) {
          simSetup.model = SXHandlerController::SX2400;
        } else
          errflg++;
        break;
      case 's':
...
//
    ...
    if(errflg) {
      ...
      fprintf(stderr, "                         SX1101, SX1701, SX2400 default: SX1101\n");
...
int main(int argc, char *argv[])
{
  ...
  SXHandlerControllerSetup   SXSimSetup;
  ViewControllerSetup        vcSetup;
  int i;
  ...
  /* init defaults */
  initializeDefaults(SXSimSetup, vcSetup);

  /* get real operational parameters */
  if(!arguments(argc, argv, SXSimSetup,vcSetup))
    exit(1);

  sleep(1);

  try {
    SXHandlerController SXhandler(&SXSimSetup,&vcSetup);
