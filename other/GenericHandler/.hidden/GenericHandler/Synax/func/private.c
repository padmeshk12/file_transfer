 ...
 *            11 Mar 2005, Ken Ward - copied for Synax GPIB
 *
 * Instructions:
 *
...
#define NO_BINNING_RESULT_SX2400  "A"
...
static const char *findalphanum(const char *s)
{
  while( *s && !isalnum(*s) ) {
    ++s;
  }
...
static char *statusString(phFuncId_t handlerID)
{
  ...
  switch( handlerID->model ) {
    case PHFUNC_MOD_SX1101:
    case PHFUNC_MOD_SX1701:
    case PHFUNC_MOD_SX2400:
      break;
...
/* End of Huatek Modification */
  ...
  switch( handlerID->model ) {
    ...
    case PHFUNC_MOD_SX2400:
      ...
      break;
    case PHFUNC_MOD_SX1701:
      // NO SPACE AFTER FULLSITES ON RESPONSE FROM 1701
      if( sscanf(handlerAnswer, "FULLSITES%lx", &population) != 1 ) {
        phLogFuncMessage(handlerID->myLogger, PHLOG_TYPE_ERROR,
                         "site population not received from handler: \n"
                         "received \"%s\" format expected \"FULLSITESxxxxxxxx\" \n",
                         handlerAnswer);
        handlerID->p.oredDevicePending = 0;
        return PHFUNC_ERR_ANSWER;
      }
                                 ...
                                 )
{
    ...
    if( handlerID->binIds )
    {
        ...
        else
        {
            ...
            phLogFuncMessage(handlerID->myLogger, LOG_DEBUG, "using binIds set binNumber %d to binCode \"%s\" ", binNumber, binCode);
        }
    }
    else
    {
        /* if handler bin id is not defined (default mapping), use the bin number directly */
        if(handlerID->model == PHFUNC_MOD_SX2400)
        {
            /* For the SX2400 model
             * Bin code in Binning Command: 0 1 2 3 4 5 6 7 8
             * Real bin code in handler:    0 1 2 3 4 5 6 7 8
             */
            /* The SmarTest bin with hardbin number 0 is mapped to the first handler 0, therefor we don't need to increase the bin number with SX2400 */
            if( binNumber > 8 )
            {
                phLogFuncMessage(handlerID->myLogger, PHLOG_TYPE_FATAL, "received illegal request for bin %lx ", binNumber);
                retVal = PHFUNC_ERR_BINNING;
            }
            else
            {
                if(binNumber == -1)
                    sprintf(binCode, "%s", NO_BINNING_RESULT_SX2400);
                else
                    sprintf(binCode, "%ld", binNumber);
                phLogFuncMessage(handlerID->myLogger, LOG_DEBUG, "using binNumber set binNumber %d to binCode \"%s\" ", binNumber, binCode);
            }
        }
        else
        {
            ...
            {
                phLogFuncMessage(handlerID->myLogger, PHLOG_TYPE_FATAL, "received illegal request for bin %lx ", binNumber+1);
                retVal = PHFUNC_ERR_BINNING;
            }
            else
            {
                              ...
                              )
{
  ...
  do {
    ...
    {
        if( i < handlerID->noOfSites && handlerID->activeSites[i] &&
                (oldPopulation[i] == PHESTATE_SITE_POPULATED || oldPopulation[i] == PHESTATE_SITE_POPDEACT) )
        {
            retVal = setupBinCode(handlerID, perSiteBinMap[i], thisbin);
            if( retVal==PHFUNC_ERR_OK )
            {
                strcat(testresults, thisbin);
                phLogFuncMessage(handlerID->myLogger, LOG_DEBUG, "will bin device at site \"%s\" to %s", handlerID->siteIds[i], thisbin);
                sendBinning = 1;
            }
            else
            {
                phLogFuncMessage(handlerID->myLogger, PHLOG_TYPE_ERROR, "unable to send binning at site \"%s\"", handlerID->siteIds[i]);
                sendBinning = 0;
            }
        }
        else
        {
            ...
            strcat(testresults, handlerID->model==PHFUNC_MOD_SX2400? NO_BINNING_RESULT_SX2400:NO_BINNING_RESULT);
            if( i < handlerID->noOfSites )
                phLogFuncMessage(handlerID->myLogger, LOG_DEBUG, "no device to bin at site \"%s\"", handlerID->siteIds[i]);
        }
        if( i == 0 )
            strcat(testresults, ";");
        else if( i % 8 == 0 )
...
static phFuncError_t doReconfigure(phFuncId_t handlerID)
{
  ...
  if( confError != PHCONF_ERR_UNDEF && confError != PHCONF_ERR_OK )
    ...
    if( found == 1 ) {
      phLogFuncMessage(handlerID->myLogger, PHLOG_TYPE_WARNING,
                       "remote stopping is not (yet) supported by Synax handler");
    }
  }
