...
/* 
 * Enumeration type for different communication interface types 
 * should reflect possible value types for the PHKEY_IF_TYPE
 */
typedef enum {
    GPIB,
    LAN,
...
/* Enumeration type for phEventSetDevicesToTest() reason for called */
typedef enum {
    SET_DEVICE_WP_TIMEOUT /* timeout during waiting for part */,
    SET_DEVICE_IF_PROBLEM /* HW interface does not work */,
    SET_DEVICE_ANSWER_PROBLEM /* plugin has not understood
                                    answer received from handler */,
    SET_DEVICE_HANDTEST     /* set device as part of handtest mode */
} setDevicesCause_t;
...
struct phEventStruct
{
    struct phEventStruct *myself;               /* self reference for validity
                                                   check */
    phFuncId_t driverId;                        /* the driver plugin ID */
    ...
    phFuncAvailability_t pfType;
    int allowHandledOperation;
    char diagnosticsMsg[PHEVENT_MAX_MESSAGE_LENGTH +1];
    int retryPluginCall;
    interfaceTypeEnum interfaceType;

    /* internally used variable for GUI button descriptions */
    BOOLEAN firstButtonCreated;
};
