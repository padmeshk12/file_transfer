...
CorruptBinDataScheme::CorruptBinDataScheme(phLogId_t l, int min, int max) : 
    LogObject(l),
    minimumBinNumber(min),
    maximumBinNumber(max),
    binDataCount(0)
{
}

int CorruptBinDataScheme::corruptBinData(int b)
{
    int corrupt;

    corrupt = minimumBinNumber + (b+binDataCount) % (maximumBinNumber - minimumBinNumber);

    if ( corrupt == b )
    {
        ++corrupt;

        if ( corrupt == maximumBinNumber )
        {
            corrupt=minimumBinNumber;
        }
    }
...
CorruptBinDataOnceScheme::CorruptBinDataOnceScheme(phLogId_t l, int c1, int min, int max) :
    CorruptBinDataScheme(l,min,max), corruptOnce(c1)
{
}
...
CorruptBinDataTwiceScheme::CorruptBinDataTwiceScheme(phLogId_t l, 
                                                     int c1, 
                                                     int c2,
                                                     int min,
                                                     int max) :
    CorruptBinDataScheme(l,min,max), corruptOnce(c1), corruptTwice(c2)
{
}
...
/*--- HandlerSetup constructor ----------------------------------------------*/

HandlerSetup::HandlerSetup()
{
    numOfSites               =   4;
    minimumBinNumber         =   0;
    maximumBinNumber         =   100;
    siteEnabledMask          =   allSitesEnabled;
    autoSetupDelay           =   0.0;
    handlingDelay            =   1000;
    numOfDevicesToTest       =   1000;
    pattern                  =   Handler::all_sites_working;
    handlerReprobeMode       =   Handler::perform_reprobe_separately;
    maxVerifyCount           =   Handler::no_max_verify_count;
    ...
    switch ( setup.corruptBinDataScheme )
    {
      ...
      case every_17th_bin_data:
        corruptBinDataScheme = new CorruptBinDataOnceScheme(l, 17,
                                                            setup.minimumBinNumber,
                                                            setup.maximumBinNumber);
        break;
      case every_11th_and_15th_bin_data:
        corruptBinDataScheme = new CorruptBinDataTwiceScheme(l,11,15,
                                                            setup.minimumBinNumber,
                                                            setup.maximumBinNumber);
        break;
...
{
    phLogSimMessage(logId, LOG_VERBOSE, 
                    "Handler::allTestsComplete() N=%d (%d) all-binned %s rep-pending %s",
                    numOfTestedDevices,
                    numOfDevicesToTest,
                    (allSitesBinned()) ? "True" : "False",
                    (reprobePending()) ? "True" : "False" );
