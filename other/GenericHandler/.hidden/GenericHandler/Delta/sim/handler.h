...
const int siteNotYetAssignedBinNumber = 0;
...
{
public:
    CorruptBinDataScheme(phLogId_t l, int min=0, int max=100);
    virtual int corruptBin(int s, int b)=0;
protected:
    int corruptBinData(int b);
    int minimumBinNumber;
    int maximumBinNumber;
    int binDataCount;
};
...
{
public:
    CorruptBinDataOnceScheme(phLogId_t l, int c1, int min, int max);
    virtual int corruptBin(int s, int b);
private:
    int corruptOnce;
};
...
{
public:
    CorruptBinDataTwiceScheme(phLogId_t l, int c1, int c2, int min, int max);
    virtual int corruptBin(int s, int b);
private:
    int corruptOnce;
    int corruptTwice;
...
public:
    ...
    enum eCorruptBinDataScheme { none,
                                 every_17th_bin_data,
                                 every_11th_and_15th_bin_data,
                                 every_17th_and_18th_bin_data };

    // verify constants

    enum { no_max_verify_count = -1 };
