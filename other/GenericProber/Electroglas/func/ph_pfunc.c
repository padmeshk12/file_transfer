...
#define RememberCall(x, c) \
    (x)->lastCall = (x)->currentCall; \
    (x)->currentCall = (c)
...
)
{
    ...
    if (!isConfigured)
    {
        ...
        tmpId->lastCall = (phPFuncAvailability_t) 0;
        tmpId->currentCall = PHPFUNC_AV_INIT;
...
)
{
	...
	if (rv == PHPFUNC_ERR_OK)
	{
		...
		type = PHESTATE_WAFT_REGULAR;
            ...
            {
		/* we are loading from another location */
...
)
{
    ...
    CheckHandle(proberID);
    RememberCall(proberID, PHPFUNC_AV_TEST);
...
)
{
    ...
    CheckHandle(proberID);
    RememberCall(proberID, PHPFUNC_AV_DIAG);
...
)
{
    ...
    CheckHandle(proberID);
    /* do not set the current call here, fake the last call instead,
       so that it is correct for the next call (call to this function
       will not be visisble) */
    /* RememberCall(proberID, PHPFUNC_AV_STATUS); */
    proberID->currentCall = proberID->lastCall;
...
)
{
    ...
    CheckHandle(proberID);
    RememberCall(proberID, PHPFUNC_AV_UPDATE);
