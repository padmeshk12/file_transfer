...
typedef enum {
    ...
    PHSTATE_LEV_CASS_STARTED          /* new cassette started */,
    PHSTATE_LEV_CASS_DONE             /* cassette finished */,
    PHSTATE_LEV_WAF_STARTED           /* new wafer started */,
    PHSTATE_LEV_WAF_DONE              /* wafer finished */,
    PHSTATE_LEV_DIE_STARTED           /* die started, ready for test */,
    PHSTATE_LEV_DIE_DONE              /* die finished, dies are
					 binned unless retesting or 
					 checking */,
    PHSTATE_LEV_SUBDIE_STARTED        /* sub-die started (if used),
                                         ready for test */,
    PHSTATE_LEV_SUBDIE_DONE           /* sub-die finished (if used),
                                         dies are binned unless
                                         retesting or checking */,
...
typedef enum {
    PHSTATE_WR_NORMAL = 0             /* normal wafer exchange operation */,
    PHSTATE_WR_RETEST                 /* retest current wafer */,
    PHSTATE_WR_REFERENCE              /* test reference wafer */,
    PHSTATE_WR_RET_REF                /* retest the reference wafer */
} phStateWaferRequest_t;
 ...
 * Description:
 * 
 * The current wafer mode is returned. Wafer modes tell the driver
 * framework about possible exceptions within wafer handling, e.g. not
 * unloading a wafer after a wafer retest was requested. The wafer
 * mode is changed inplicetly at the end of pause actions, the wafer
 * unload actions, and the wafer load actions. The RETEST_W and
 * REFERENCE flags of SmarTest are read to achieve this.
 *
 * Returns: error code
 *
 ***************************************************************************/
 ...
 * Description:
 * 
 * The current wafer mode can be set (see also
 * phStateGetWaferMode(3)). Usually this function is not called
 * directly, since the wafer modes are set implicetly by calls to
 * phStateChangeLevel(3). This function may be used to bring the
 * driver into a special state for debug purposes or at startup. Also
 * user interfaces may use this function to change the mode.
 *
