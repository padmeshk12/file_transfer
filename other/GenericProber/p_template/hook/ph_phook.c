 ...
 *
 * HISTORY  : 24 Aug 1999, Michael Vogt, hook template created
 *
 * Description:
...
)
{
	...
	"phHookInit(P%p, P%p, P%p, P%p, P%p, P%p, P%p)",
...
)
{
    ...
    /* Here is a sample implementation which prints out a message on
       each incoming ph_die_done call (before the framework starts
       to bin the dies) */
	...
	phLogHookMessage(hook.myLogger, PHLOG_TYPE_MESSAGE_0,
	    "start of ph_die_start recognized by hook function");

	/* set the result value:
	   the framework must continue with the ph_die_start actions */
	*result = PHEVENT_RES_CONTINUE;
	...
	phLogHookMessage(hook.myLogger, PHLOG_TYPE_MESSAGE_0,
	    "start of ph_die_done recognized by hook function");

	/* set the result value:
	   the framework must continue with the ph_die_done actions */
	*result = PHEVENT_RES_CONTINUE;
	break;
...
)
{
    ...
    /* Here is a sample implementation which prints out a message after
       each performed ph_die_start call */

    switch (call)
    {
      case PHFRAME_ACT_DIE_START:

	/* get the handler's site population status and count all dies */
	...
	phLogHookMessage(hook.myLogger, PHLOG_TYPE_MESSAGE_0,
	    "end of ph_die_start recognized by hook function, total: %ld",
	    dieCount);
	break;
	...
	phLogHookMessage(hook.myLogger, PHLOG_TYPE_MESSAGE_0,
	    "end of ph_die_done recognized by hook function");
	break;
