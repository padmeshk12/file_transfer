 ...
 *              Implement the "phPFuncGetStatus". "getStatusXXX" functions are
 *              used to retrieve information/parameter from Prober.
 *            February 2006, Garry Xie , CR28427
 *              Implement the "phPFuncSetStatus" which is one of the "setStatusXXX" 
 *              serial functions that are used to set parameters to Prober.
 *            July 2006, Fabarca & Kun Xiao, CR21589
 *              Implement the "phPFuncContacttest"
 *            Nov 2008, CR41221 & CR42599
 *              Implement the function "phPFuncExecGpibCmd" and
 *              "phPFuncExecGpibQuery"
 *
 * Instructions:
 *
...
phPFuncAvailability_t phPFuncAvailable(void)
{
    ...
    retval = (phPFuncAvailability_t)(retval | PHPFUNC_AV_GETSTATUS);
#endif
#ifdef SETSTATUS_IMPLEMENTED
    retval = (phPFuncAvailability_t)(retval | PHPFUNC_AV_SETSTATUS);
#endif
#ifdef DESTROY_IMPLEMENTED
    retval = (phPFuncAvailability_t)(retval | PHPFUNC_AV_DESTROY);
#endif

/*CR21589, kunxiao, 16 Jun 2006*/
#ifdef CONTACTTEST_IMPLEMENTED
    retval = (phPFuncAvailability_t)(retval | PHPFUNC_AV_CONTACTTEST);
#endif
 ...
 * 
 ***************************************************************************/
phPFuncError_t phPFuncGetStatus(
    phPFuncId_t proberID        /* driver plugin ID */,
...
)
{
  ...
  CheckInitialized();
  phLogFuncMessage(myLogger, PHLOG_TYPE_TRACE, 
                   "phPFuncGetStatus(P%p, P%p)", proberID, commandString);
  phLogFuncMessage(myLogger, PHLOG_TYPE_TRACE, 
                   "phPFuncGetStatus(P%p, %s)", proberID, commandString);
  CheckHandle(proberID);
  RememberCall(proberID, PHPFUNC_AV_GETSTATUS);


  /* this is dummy code, only performed during regression tests */
  if ( DummyOperation(proberID, "get prober status") ) {
    phPFuncError_t rv = DummyReturnValue(proberID);
    if( rv == PHPFUNC_ERR_OK) {
      strcpy(response, "dummy get status string");
  ...
  /* complete the call in private section */
  return privateGetStatus(proberID, commandString, responseString);
}
#endif
...
/*****************************************************************************
 *
 * Set status ( parameter, information,etc.) to Prober by key name
 *
 * Authors: Garry xie
 *
 * Description:
 ...
 * 
 ***************************************************************************/
phPFuncError_t phPFuncSetStatus(
    phPFuncId_t proberID        /* driver plugin ID */,
    char *commandString         /* key name to get parameter/information */,
    char **responseString       /* output of response string */
)
{
  ...
  CheckInitialized();
  phLogFuncMessage(myLogger, PHLOG_TYPE_TRACE, 
                   "phPFuncSetStatus(P%p, P%p)", proberID, commandString);
  phLogFuncMessage(myLogger, PHLOG_TYPE_TRACE, 
                   "phPFuncSetStatus(P%p, %s)", proberID, commandString);
  CheckHandle(proberID);
  RememberCall(proberID, PHPFUNC_AV_SETSTATUS);


  /* this is dummy code, only performed during regression tests */
  if ( DummyOperation(proberID, "set prober status") ) {
    phPFuncError_t rv = DummyReturnValue(proberID);
    if( rv == PHPFUNC_ERR_OK) {
      strcpy(response, "dummy set status string");
      *responseString = response;
    }
  ...
  /* end of dummy code */
  strncpy(input, commandString, MAX_STATUS_MSG_LEN-1);
  //if autoz token is found, step in setStatusForAutoz in private.c
  char *autozToken = strstr(input,"AUTOZ_");
  if(autozToken)
  {
    token = strtok(input, " \t\n\r");
    ...
    if ( token )
    {
      ...
      retVal = privateSetStatusForAutoZ(proberID, token, parms, &answer);
    }
    else
    {
      retVal = PHPFUNC_ERR_NA;
    }

    if(answer != NULL)
    {
      strncpy(response, answer, MAX_STATUS_MSG_LEN-1);  // ensure no overflow
      strcat(response, "\0");  // ensure null terminated
      *responseString = response;
      phLogFuncMessage(myLogger, PHLOG_TYPE_MESSAGE_0,
          "phFuncGetStatus, response ->%s<-, length = %ld",
  ...
  {
     /* complete the call in private section */
     return privateSetStatus(proberID, commandString, responseString);
  }
...
/*****************************************************************************
 *
 * set the prober Z height so to pass continuity
 *
 ...
 *
 ***************************************************************************/
phPFuncError_t phPFuncContacttest(
    phPFuncId_t proberID                         /* driver plugin ID */,
...
)
{
    ...
    CheckInitialized();
    phLogFuncMessage(myLogger, PHLOG_TYPE_TRACE,
                     "phPFuncContacttest(P%p)", proberID);
    CheckHandle(proberID);
    RememberCall(proberID, PHPFUNC_AV_CONTACTTEST);
    if (DummyOperation(proberID, "set prober Z height"))
    {
        /* this is dummy code, only performed during regression tests */
        phPFuncError_t rv;
    ...
    /* complete call in private section */
    return privateContacttest(proberID, contacttestSetupID);
}
#endif /* CONTACTTEST_IMPLEMENTED */
