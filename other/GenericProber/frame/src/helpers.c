      ...
      if (askForLog){
        if( (*f)->enableDiagnoseWindow == YES ) {
          phEventPopup((*f)->eventId, &result, &stReturn, 0, "menu",
                       "Abort detected by prober driver:\n"
                       "Do you want to...\n"
          ...
          phLogFrameMessage((*f)->logId, PHLOG_TYPE_MESSAGE_0, "Abort detected by prober driver\n\n");
        }
      }
      askForLog = 0;
...
int phFrameCheckInit(struct phFrameStruct **f)
{
    ...
    *f = initDriverFramework();
	...
	{
	    phLogFrameMessage((*f)->logId, PHLOG_TYPE_FATAL,
	        "prober driver will not work\n"
		BIG_FATAL_STRING
		"\nAll further calls to the prober driver are ignored\n\n"
	...
	{
	    fprintf(stderr, 
		"PH lib: FATAL! Prober driver will not work\n"
                BIG_FATAL_STRING
		"\nAll further calls to the prober driver are ignored\n"
  ...
  do{
    if (!wasDumped){
      if( frame->enableDiagnoseWindow == YES ) {
        ...
        phLogFrameMessage(frame->logId, PHLOG_TYPE_MESSAGE_0, "Prober driver PANIC!!\n");
        /* 
